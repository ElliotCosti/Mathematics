...
SolveSixMinus := function (G: SpecialGroup := false)
   ...
   // assert forall{i :i in [1..2] | InnerProduct (J[i] * form, J[i]) eq 0};
   ...
   Hb`SLPGroupHom := hom <Hb`SLPGroup -> Hb | UserGenerators (Hb)>;

   /* rewrite involution u wrt this new basis */
   b := GL(d, q) ! (NB * H`SLPGroupHom (wu) * NB^-1);

   Cb := CentraliserOfInvolution (Hb, b, wu);

   Cb := SOGoodCentraliser (Hb, Cb, [4], {5..6}: Words :=true, 
                           SpecialGroup:=SpecialGroup);
   T := ExtractFactor (Cb, [1..4]);
   if SpecialGroup eq true then 
      ET, WT, CBT := SolveSOPlus (T);
   else 
      ET, WT, CBT := SolveOPlus (T);
   end if;
   assert IsIdentity (CBT);
   // assert Verify (T, ET, WT, CBT);

   E := MinusChosenElements (OmegaMinus (6, q): Words := false); 

   tau := hom < T`SLPGroup -> Cb`SLPGroup | 
          [Cb`SLPGroup.0] cat [Cb`SLPGroup.i: i in [1..Ngens (Cb`SLPGroup)]]>;
