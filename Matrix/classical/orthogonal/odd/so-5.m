...
FiveSplitSpace := function (G : Limit := 100)
...
SolveO5 := function (G)
   ...
   vprint User5: "Time taken to solve O+4", Cputime (t4);

   tau := hom < T4[1]`group`SLPGroup -> C4`SLPGroup | 
                [C4`SLPGroup.i: i in [1..Ngens (C4`SLPGroup)]]>;
   ...
   for i in [1..#A1] do
      ...
      flag, w := SolveWord (T4, T4[1], a);
      w := tau (w);
      W1[i] := FactorToParent (H, C4, w);
   end for;
   // assert Verify (H, A1, W1, H.1^0);
   ...
   t4 := Cputime ();
   E4, W4, CB4 := SolveOPlus (S);
   vprint User5: "Time taken to solve O+4", Cputime (t4);

   assert IsIdentity (CB4);
 
   gamma := hom <S`SLPGroup -> H`SLPGroup | [H`SLPGroup.0] cat W1 cat W2>;
   w := gamma (W4[#W4]);
   
   E := SOChosenElements (H: Words := false);

   return E, W3 cat [w], GL(5, F) ! (CB * cob);
end function;
