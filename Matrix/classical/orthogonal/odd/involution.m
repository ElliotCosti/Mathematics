...
/* is g a suitable involution for Omega^- case?
   ...
   and Omega^-? if SpecialGroup then we are working with SO^- */ 
...
IsMinusInvolution := function (G, g, U, W: SpecialGroup := false)
   ...
   error "Should not be here in IsMinusInvolution";
...
SOSplitSpace := function (G : Limit := 300, 
   ...
   repeat 
      ...
      vprint User5: "Time to construct SOSplitSpace involution is ", Cputime (t);
      if flag then 
         U := Eigenspace (g, 1);
         W := Eigenspace (g, -1);
   ...
   if nmr gt Limit then 
      "Range is ", Range; G:Magma; error "Failed to split space"; 
   end if;
