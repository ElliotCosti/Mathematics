 ...
 */
  ...
  H := sub< GL(d,q) | (Random(G),Random(G)), (Random(G),Random(G)),
                                                 (Random(G),Random(G)) >;
  oldl := d;
  done := false;
  ct := 0;
  repeat
    ...
    if l eq 1 then
      if IsAbsolutelyIrreducible(H) then
        ...
        H := sub< GL(d,q) | H, (Random(G),Random(G)) >;
        ct +:= 1;
      end if;
    else
      H := sub< GL(d,q) | H, (Random(G),Random(G)) >;
...
// Decide whether G preserves a symmetric bilinear form.
   ...
   * in mat 1 */
          ...
          if IsOdd(q) then
              ...
              sign:= ClassicalForms_Signum(GF(q), mat);
          else
              gens:= SetToSequence(Generators(G));
              scals:= [Identity(G) : i in [1..#gens]];
              quad:= ClassicalForms_QuadraticForm2(GF(q), mat, gens, scals);
...
intrinsic SymmetricBilinearForm(G::GrpMat : Scalars:=false ) -> BoolElt, AlgMatElt, RngIntElt
{Assuming that G acts absolutely irreducibly, try to find a symmetric
bilinear form which is G-invariant (modulo scalars if Scalars is true) or
prove that no such form exists.}

  local M, D, bool, mat, d, q, sign;
  if not IsIrreducible(G) then
    error "SymmetricBilinearForm: G must be irreducible";
  end if;
  d := Dimension(G);
  ...
  if bool then
   if mat eq Transpose(mat) then
    ...
    if IsOdd(q) then
      //sign:= IsEven(d) select ClassicalForms_Signum(GF(q), mat, 0) else 0;
      sign:= IsEven(d) select ClassicalForms_Signum(GF(q), mat) else 0;
    else
      gens:= SetToSequence(Generators(G));
      scals:= [Identity(G) : i in [1..#gens]];
      quad:= ClassicalForms_QuadraticForm2(GF(q), mat, gens, scals);
      if quad cmpeq false then
        ...
        if Scalars then return bool, mat, 0, [mat[1][1]^0 : i in [1..Ngens(G)]];
        else return bool, mat, 0;
        end if;
      end if;
      sign:= ClassicalForms_Signum2(GF(q), mat, quad);
    end if;
    if Scalars then return bool, mat, sign, [mat[1][1]^0 : i in [1..Ngens(G)]];
    else return bool, mat, sign;
    end if;
   end if; //if mat eq Transpose(mat)
   if not Scalars then
     if IsAbsolutelyIrreducible(G) then
...
// Decide whether G preserves a quadratic form
  ...
  if bool then
   if mat eq Transpose(mat) then
    ...
    if IsOdd(q) then
        //sign := IsOdd(q) select ClassicalForms_Signum(GF(q), mat, 0) else 0;
        sign := IsOdd(q) select ClassicalForms_Signum(GF(q), mat) else 0;
        for i in [2..d] do for j in [1..i-1] do
           mat[i][j] := 0*mat[i][j];
        end for; end for;
        for i in [1..d] do mat[i][i] /:= 2; end for;
        if Scalars then
          return bool, mat, sign, [mat[1][1]^0 : i in [1..Ngens(G)]];
        else return bool, mat, sign;
        end if;
    end if;
    gens:= SetToSequence(Generators(G));
    ...
    if Scalars then return bool, quad, sign, [mat[1][1]^0 : i in [1..Ngens(G)]];
    else return bool, quad, sign;
    end if;
   end if; //if mat eq Transpose(mat)
   if not Scalars then
     if IsAbsolutelyIrreducible(G) then
...
//Decide whether G preserves unitary form.
  ...
  if bool then
    if mat ne Transpose(MatToQ(mat, q)) then
      c := Representative(
          { <i,j>: i in [1..d], j in [1..d] | mat[i][j] ne GF(q^2)!0 } );
...
which is G-invariant (modulo scalars if Scalars is true) or prove that no
such form exists.}
  local bool, form, scals, scalsq, sign, signq;
  if not IsIrreducible(G) then
  ...
  if Scalars then
     bool, form, sign, scals := SymmetricBilinearForm(G:Scalars:=true);
     if bool cmpeq "unknown" then
       ans`formType := "unknown";
     elif bool then
       ans`bilinearForm := form;
       bool, form, signq, scalsq := QuadraticForm(G:Scalars:=true);
       if bool cmpeq "unknown" or bool eq false then
         ...
         if signq eq 1 then ans`formType := "orthogonalplus";
         elif signq eq -1 then ans`formType := "orthogonalminus";
         elif signq eq 0 then ans`formType := "orthogonalcircle";
         end if;
     ...
     bool, form, sign := SymmetricBilinearForm(G);
     if bool cmpeq "unknown" then
       ans`formType := "unknown";
     elif bool then
       ans`bilinearForm := form;
       bool, form, signq := QuadraticForm(G);
       if bool cmpeq "unknown" or bool eq false then
         //should only happen in characteristic 2
         ans`formType := "symplectic";
