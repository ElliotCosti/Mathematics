...
ProjectiveGenerator := function (G, g, wg: Limit := 100)
   nmr := 0;
   repeat 
...
GoodCentraliser := function (P, G, desired, list: type := "linear",
   IsCorrectType := MyIsLinearGroup, Words := true, Limit := 500) 
   ...
   repeat 
      ...
      // vprint User5: "Non-scalar found ", required;
...
*/
   A := [a^0, a];
   Large := [g^0, g];
   if Words then wg := wg^(order); WA := [wg^0, wg];end if;
...
SpecialCentraliser := function (G, g, wg, f: fct := Order, 
   ...
   repeat 
      ...
      i +:= 1;
   until flag or i gt 100;
 if i gt 100 then C:Magma; error "HERE"; end if;
...
ThirdCentraliser := function (G, g, wg: 
         type := "linear", IsCorrectType := MyIsLinearGroup)
...
SplitSpace := function (G : Limit := 50, SortSpaces := true,
...
SplitInvolution := function (G: Required := Degree (G) div 2, 
        Scalars := false, IsCorrectType := MyIsLinearGroup)
   ...
   if e le Minimum ([minus, plus]) then 
      ...
      C1 := GoodCentraliser (G, C, plus, {plus + 1..d}:
                             IsCorrectType := IsCorrectType); 
      G1 := ExtractFactor (C1, {1..dim});
      g1, w1, smallb1, SmallCB1 := $$ (G1: Scalars := false, 
     ...
     error "Should not be here in SplitInvolution";
  end if;
