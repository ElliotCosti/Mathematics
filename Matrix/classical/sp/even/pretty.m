...
[Degree (x): x in S];
if exists(x){x : x in S | Degree (x) eq 4} then 
x:Magma; end if;
[ ClassicalType (x): x in S];
/* 
for x in S do 
RecognizeClassical (x);
ClassicalType (x);
end for;
*/
         if exists (x) {x : x in S | Degree (x) eq expected and
                                     RecognizeClassical (x) 
and ClassicalType (x) in {"symplectic", 
 "orthogonalminus", 
"orthogonalplus"}} then
            wx := W[i]; wy := W[j];
...
/* find SL(e, q) as subgroup of SL(d, q) for some e in rank */
