import "C:/Program Files/magma/package/Group/GrpMat/ClassicalRec/sld2.m":
...
MyIsUnitaryGroup := function (G)
   ...
   if Degree (G) eq 2 then
      if #F eq 9 then 
         if IdentifyGroup (G) in [<24, 3>, <96, 67>] 
            then return true; else return false; end if;
      elif #F eq 25 then
         if #G eq 120 or #G eq 720 then return true; else return false; end if;
      elif #F eq 81 then 
         if #G ge 720 then return true; else return false; end if;
      else 
         flag, id := SimpleGroupName (G);
         if not flag then return false; end if;
