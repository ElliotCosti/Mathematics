...
if r eq 1 then
   S := (SS.2^(SS.3*((SS.4*SS.3)^(j-i-1))*(SS.4^(i-1)))) ^Z!Eltseq(A[j][i])[r]*S;
   // aa := (t^(u*((v*u)^(j-i-1))*(v^(i-1)))) ^Z!Eltseq(A[j][i])[r]*A;
...
if IsEven(r) then
   S := (((OO^(SS.1^-((r-2) div
   2)))^(SS.3*((SS.4*SS.3)^(j-i-1))*(SS.4^(i-1)))))^Z!Eltseq(A[j][i])[r]*S;
   // aa := (((O^(delta^-((r-2) div 2)))^(u*((v*u)^(j-i-1))*(v^(i-1)))))^Z!Eltseq(A[j][i])[r]*A;
...
if IsOdd(r) then
   S := ((SS.2^(SS.1^-((r-1) div
    2)))^(SS.3*((SS.4*SS.3)^(j-i-1))*(SS.4^(i-1))))^Z!Eltseq(A[j][i])[r]*S;
   // aa := ((t^(delta^-((r-1) div 2)))^(u*((v*u)^(j-i-1))*(v^(i-1))))^Z!Eltseq(A[j][i])[r]*A;
   AddRow(~A, -Eltseq(A[j][i])[r] * w^(r-1), i, j);
end if;
...
if IsEven(r) then
   S := S*((OO^(SS.1^-((r-2) div 2)))^(((SS.4*SS.3)^(j-i-1))*(SS.4^(i-1))))^-Z!Eltseq(A[i][j])[r];
   // aa := A*((O^(delta^-((r-2) div 2)))^(((v*u)^(j-i-1))*(v^(i-1))))^-Z!Eltseq(A[i][j])[r];
   AddColumn(~A, -Eltseq(A[i][j])[r] * w^(r-1), i, j);
end if;

if IsOdd(r) then
   S := S*((SS.2^(SS.1^-((r-1) div 2)))^(((SS.4*SS.3)^(j-i-1))*(SS.4^(i-1))))^-Z!Eltseq(A[i][j])[r];
   // aa := A*((t^(delta^-((r-1) div 2)))^(((v*u)^(j-i-1))*(v^(i-1))))^-Z!Eltseq(A[i][j])[r];
   AddColumn(~A, -Eltseq(A[i][j])[r] * w^(r-1), i, j);
end if;

end procedure;

// A*(v^-1) puts the first column to the back, shuffles each column left one and negates all the others
// A*((v^-1)^k) where k is even, puts the first k columns to the back and negates them whilst shifting all the others to the left
// A*((v^-1)^k) where k is odd, puts the first k columns to the back and negates all the OTHER columns whilst shifting these others to the left
...
// A*v brings the last column to the front, shuffles each other row right one place whilst negating them
// A*((v^-1)^k) where k is even, brings the last k columns to the front and negates them whilst shifting all the others to the right
// A*((v^-1)^k) where k is odd, brings the last k columns to the front and negates all the OTHER columns whilst shifting these others to the right
// v^-1 * A brings the bottom row to the top, shuffles each other row down one place whilst negating them
...
// A := A^((v^-1)^k); // moves the first, second, ..., kth column and row out of the way.
ConjByVInverse(~A, k);
...
if A[1, 1] eq 0 then
i := Depth(A[1]);
// A := A*(u*v^-1)^(i-2)*(u*v)^(i-2)*u; // swaps column 1 with column i whilst negating every column between 1 and i exclusive. In the special case i = 2, the negation is only applied to column 1. This will never move any columns that have already been done because they will always contain a 0 in the first row at this stage.
A := SwapColumns(A, 1, i);
if i ne 2 then
   for j in [2..(i-1)] do
...
if A[1, 1] ne 1 then
   ...
   if {A[i, 1] eq 0 : i in [2..d]} eq {true} then
      // A := A*(t^u);
// this subtracts column 2 from column 1 and is necessary if the column has zeroes in every position as else we will not be able to subsequently add a multiple of one row to the top row in order to get a 1 in the [1, 1] position.
      AddColumn(~A, -1, 2, 1);
      S2 := S2*(SS.2^SS.3);
   end if;
   while A[i, 1] eq 0 do
...
for j in [2..d-k] do
for r in [1.. e] do // needs to go up to the power of the characteristic of the field
   RowOp(1, j, ~A, ~S1, r);
end for;
end for;
...
// A := A^(v^k); // moves the rows and columns back to their original position
ConjByV(~A, k);
