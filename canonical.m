...
VectorCF := function (X, XSLP, u: ComputeBase := ComputeBase)
   ...
   while (j0 lt d + 1) do
      ...
      /* if we are not assuming that X forms the required chief series,
         we remake X a chief series here */
      if ComputeBase then
         X, XSLP := PChiefSeriesGenerators(X, XSLP);
      end if;
...
SubspaceCF := function (X, XSLP, U: ComputeBase := ComputeBase)
   ...
   for i in [t - 1..1 by -1] do
      ...
      temp, tempslp, U, depths, X, XSLP := NextSubspaceCF (X, XSLP, V, U,
                                   depths, i: ComputeBase := ComputeBase);
...
PGroupStabiliser := function (S, U, W, psi: ComputeBase := ComputeBase)
   ...
   /* gets S as a subgroup of GL(d, F); unclear as to why it needs to
      do this as S is already in this form. */
   S := sub <GL (d, F) | S >;

   /* trivial case */
   if Ngens (S) eq 0 then 
      return U, Identity (GL(d, F)), Identity(W), S, 1; 
   end if;
   ...
   if ComputeBase then
      S, XSLP := PChiefSeriesGenerators(S, XSLP: initial := true);
   end if;
   ...
   return UC^CBinv, Trans * CBinv, transslp, 
      sub <GL (d, F) | [Y[i]^TransInv : i in [1..#Y]]>, length;
...
function MyUnipotentStabiliser (G, U: ComputeBase := true)

   W, psi := WordGroup(G);
   cf, trans, transslp, stab := PGroupStabiliser (G, U, W, psi: 
      ComputeBase := ComputeBase);
